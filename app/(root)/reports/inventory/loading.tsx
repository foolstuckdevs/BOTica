            ...
            <div className="border-b bg-gray-50 dark:bg-gray-800/50 p-2">
              <div className="grid grid-cols-5 gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-16 bg-muted rounded-lg animate-pulse shimmer flex items-center justify-center"
                    style={{ animationDelay: `${i * 100}ms` }}
                  >
                    <div className="w-5 h-5 bg-muted-foreground/20 rounded animate-pulse shimmer" />
                  </div>
                ))}
              </div>
            </div>
