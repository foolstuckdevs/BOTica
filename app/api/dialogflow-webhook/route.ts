...
export async function POST(req: Request) {
  try {
    ...
    const params = (body.queryResult?.parameters ?? {}) as Record<string, unknown>;
    ...
    const getString = (obj: Record<string, unknown>, key: string): string | undefined => {
      ...
      if (Array.isArray(v)) {
        const first = (v as unknown[]).find((x): x is string => typeof x === 'string' && (x as string).trim().length > 0);
        return first;
      }
      return undefined;
    };
    ...
    if (intent === 'Check Stock' && queryText) {
      ...
      const findMatches = async (needle: string, limit = 8) => {
        const rows = await db
          ...
          .where(
            and(
              sql`(${products.name} ILIKE ${'%' + needle + '%'} OR ${products.brandName} ILIKE ${'%' + needle + '%'} OR ${products.genericName} ILIKE ${'%' + needle + '%'})`,
      ...
      if (matches.length === 0) {
        ...
        const cleaned = queryText
          ...
          .trim();
        const stop = new Set([
          'do', 'we', 'have', 'in', 'stock', 'the', 'a', 'an', 'is', 'there', 'any', 'of', 'for', 'and', 'please', 'medicine', 'meds',
        ]);
      ...
      if (matches.length === 1) {
        ...
        if (item.dosageForm) fulfillmentText += ` Dosage form: ${item.dosageForm.toLowerCase()}.`;
        if (item.unit) fulfillmentText += ` Unit: ${item.unit.toLowerCase()}.`;
        if (item.brandName) fulfillmentText += ` Brand: ${item.brandName}.`;
        if (item.lotNumber) fulfillmentText += ` Batch: ${item.lotNumber}.`;
  if (item.expiryDate) fulfillmentText += ` Exp: ${item.expiryDate}.`;
        ...
        const lines = shown.map((m, i) => {
          ...
          const batch = m.lotNumber ? `, batch ${m.lotNumber}` : '';
          const exp = m.expiryDate ? `, exp ${m.expiryDate}` : '';
          return `${i + 1}) ${m.name}${desc}${batch}${exp}: ${m.quantity} in stock`;
        });
        const extra = matches.length > maxList ? `\n...and ${matches.length - maxList} more. Please specify brand or dosage form.` : '';
        fulfillmentText = `I found ${matches.length} matches:\n${lines.join('\n')}${extra}`;
      }
    }
